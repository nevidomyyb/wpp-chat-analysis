...
class MessageCounts:
    ...
    def __init__(self,):
        st.set_page_config(page_title='Message Counts', page_icon='🧿', layout='wide')
        sidebar_func('messages counts page')
    def _calc_var(self, average, df):
        values = df['quantity'].values.tolist()
        sum_all = sum((x-average)**2 for x in values)
    ...
    def draw(self):
        ...
        variance = self._calc_var(average, df)
